...
class PasswordInfoDAO extends DelegableAuthInfoDAO[PasswordInfo] with DAOSlick{
  ...
  protected def passwordInfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbPasswordInfo <- slickPasswordInfos if dbPasswordInfo.userID === dbLoginInfo.userID
  ...
  protected def passwordInfoSubQuery(loginInfo: LoginInfo) =
    slickPasswordInfos.filter(_.userID in loginInfoQuery(loginInfo).map(_.userID))
  ...
  def save(loginInfo: LoginInfo, authInfo: PasswordInfo): Future[PasswordInfo] = {
    val query = loginInfoQuery(loginInfo).joinLeft(slickPasswordInfos).on(_.userID === _.userID)
